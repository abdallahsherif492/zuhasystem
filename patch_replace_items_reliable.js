...
const startStr = '{order.order_items.map(item => (';
const endStr = '{/* Add Extra Item */}';
...
// We want to replace from startIndex up to the end of the </div> before endStr
// The string before endStr is \n                                        </div>\n                                        \n                                            {/* Add Extra Item */}
// So we replace up to just before \n                                        </div>
const replacementEndIndex = code.lastIndexOf('</div>', endIndex) + '</div>'.length;
                                                                        ...
                                                                        if (v) {
                                                                            ...
                                                                            setOrders(prev => prev.map(o => {
                                                                                if (o.id === order.id) {
                                                                                    ...
                                                                                    return newOrderObj as any;
                                                                                }
                                                                                return o;
                                                                            }));
                                                                            handleUpdateItem(item.id, { variant_id: val, price_at_sale: v.sale_price }).then(() => {
                                                                                if (updatedOrder) {
                                                                                    handleUpdateOrder(order.id, { subtotal: (updatedOrder as any).subtotal, total_amount: (updatedOrder as any).total_amount });
                                                                                }
                                                                            });
                                                                        }
                                                                    }}
...
const newCode = code.substring(0, startIndex) + newItemsMapping + code.substring(replacementEndIndex);

fs.writeFileSync('src/app/(dashboard)/easy-orders/page.tsx', newCode);
console.log("Successfully replaced block using start/end logic.");
