...
export default function ShippingPage() {
    ...
    const updateRate = (gov: string, value: string) => {
        ...
        if (isNaN(num)) {
            // Remove key if empty? or set to 0? let's assume empty means remove/undefined
    ...
    const applyStandardRate = (rate: number) => {
        ...
        GOVERNORATES.forEach(g => {
            // Only apply if not already set? Or overwrite all? 
            // Typically "Standard Rate" implies overwrite.
            newRates[g] = rate;
        });
        setFormData({ ...formData, rates: newRates });
    };
